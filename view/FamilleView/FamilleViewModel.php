...
function LoadTableFamilleList(){
  ...
  for(let i = 0; i < viewQuantity; i++){
    if(familles[listPosition]){
      ...
      rows += '<input placeholder="Code" value="' + famille.CodeFamille + '" class="form-control" oninput="ToUpdate(' + updateParams + ')" id="InputCode' + famille.CodeFamille + '">';
      rows += '</td>';
...
function UpdateFamille(position, id){
  $('#UpdateFamilleButton' + id).prop('disabled', true);
...
function ToUpdate(position, id){
  if(!listLoading){
    famille = familles[position];
    if(famille.IdFamille != id){
      famille = familles.filter(x => x.IdFamille == id)[0];
    }
    MaintainFamille(famille);
    if(CheckFamille(famille)){
      $('#UpdateFamilleButton' + id).prop('disabled', false);
