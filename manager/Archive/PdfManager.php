...
use \SrgConcept\Vendor\Spipu\Html2Pdf\Html2Pdf;
use \SrgConcept\Vendor\Spipu\Html2Pdf\Exception\Html2PdfException;
use \SrgConcept\Vendor\Spipu\Html2Pdf\Exception\ExceptionFormatter;
...
class PdfManager
{
	...
	public function GenerateDevisPDF($id){
		$DevisManager = new DevisManager($this->_Db);
	  if( !isset($result['error']) ) {
	    ...
	    $clientModel = new Client($devis['client']);
	    $userModel = new User($devis['user']);
			$entrepriseModel = new Entreprise($devis['user']['entreprise']);
	    ...
	    $dataPDF .= "<th style='width: 30%';>Piece</th>";
